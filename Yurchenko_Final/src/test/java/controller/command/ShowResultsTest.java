...
public class ShowResultsTest {
    ...
    @InjectMocks
    ShowResults showResultsCommand;
    ...
    public void shouldShowResultsOfTest() {
        ...
        doNothing().when(testService).addTestToDatabase(any());
        doNothing().when(request).setAttribute(any(), anyString());

        CommandResult commandResult = showResultsCommand.execute(request, response);
        assertEquals(CommandPages.SHOW_RESULTS, commandResult.getPage());

        verify(userService).setRank(anyString(), anyInt(), anyInt());
        verify(testService).addTestToDatabase(any());
    }
